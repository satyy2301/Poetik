...
const WriteScreen = ({ navigation }: WriteScreenProps) => {
    ...
    (async () => {
      ...
      if (draft) {
        ...
        if (draft.themes?.length) {
          setSelectedThemes(draft.themes);
        }
      }
      try {
  ...
  const handleStatsChange = useCallback(
    (stats: { words: number; syllables: number }) => {
  ...
  const handleRestoreVersion = (version: DraftVersion) => {
    ...
    Alert.alert(
      'Restored',
      `Version ${version.version_number} loaded into editor.`,
    );
  };
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      await submitPoemForReview(
        published.id,
        published.title,
        published.content,
        user.id,
      );
      await logActivity(user.id, 'published', 'poem', published.id, {
        title: published.title,
      });
      await saveVersion(
        user.id,
        { title, content, form: selectedForm },
        published.id,
      );
        ...
        {!headerHidden && (
          <View style={styles.header}>
            ...
            <View style={styles.headerActions}>
              {saveStatus === 'saved' && (
                <Text
                  style={[theme.typography.bodySm, { color: colors.success }]}
                >
                  Saved
                </Text>
              )}
              ...
              >
                <Ionicons
                  name={focusMode ? 'expand' : 'contract'}
                  size={22}
                  color={colors.brandPrimary}
                />
              </TouchableOpacity>
            ...
            <View style={styles.metaRow}>
              ...
              {selectedForm && (
                <Text
                  style={[
                    theme.typography.bodySm,
                    { color: colors.brandPrimary, fontFamily: 'Inter-Bold' },
                  ]}
                >
                  {selectedForm}
                </Text>
                  ...
                  >
                    {AI_TEMPLATES.map(t => (
                      <TouchableOpacity
                        ...
                        style={[
                          styles.aiChip,
                          aiTemplate === t.key && styles.aiChipActive,
                        ]}
                      ...
                      >
                        <Text
                          style={[
                            styles.aiChipText,
                            aiTemplate === t.key && styles.aiChipTextActive,
                          ]}
                        >
                          {t.label}
                        </Text>
                  ...
                  <View style={styles.aiParams}>
                    <View style={styles.paramField}>
                      ...
                      <TextInput
                        ...
                        onChangeText={t =>
                          setAiTemperature(
                            Math.max(0, Math.min(1, parseFloat(t) || 0)),
                          )
                        }
                      />
                    </View>
                    <View style={styles.paramField}>
                      ...
                      <TextInput
                        ...
                        onChangeText={t =>
                          setAiMaxTokens(
                            Math.max(50, Math.min(2000, parseInt(t || '400'))),
                          )
                        }
                      />
                    </View>
                  ...
                  {aiSuggestions.length > 0 ? (
                    <View style={styles.suggestionsBox}>
                      ...
                      <View style={styles.suggestionFooter}>
                        ...
                        <TouchableOpacity
                          onPress={undoSuggestion}
                          disabled={!undoStack.length}
                        >
                          <Text
                            style={[
                              styles.footerBtn,
                              !undoStack.length && styles.footerDisabled,
                            ]}
                          >
                            Undo
                          </Text>
            ...
            <ScrollView>
              ...
              <View style={styles.chipGrid}>
                {POEM_CATEGORIES.map(theme => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.chip,
                      selectedThemes.includes(theme) && styles.chipSelected,
                    ]}
              ...
              <View style={styles.modalActions}>
                ...
                <TouchableOpacity
                  style={[
                    styles.confirmBtn,
                    isPublishing && styles.publishDisabled,
                  ]}
